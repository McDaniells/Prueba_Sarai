...
namespace grupomas
{
    class Make
    {
        ...
        private int UserId;
        public Make(QueryFactory db_con, int userid)
        {
        ...
        public void Select()
        {
            try
            {
                ...
                {
                    var query = con.Select("exec SP_Make_sel @ID", new
                    {
                        ID = id
                    }, transaction);
                ...
                bitacora.Insert($"Select id: {id}", "SP_Make_sel", UserId);
        ...
        public void Delete()
        {
            try
            {
                ...
                using (var transaction = con.Connection.BeginTransaction())
                {
                    var query = con.Select("exec SP_Make_del @ID", new
                    {
                        ID = id
                    }, transaction);
                ...
                bitacora.Insert($"Delete id: {id}", "SP_Make_del", UserId);
        ...
        public void Insert()
        {
            try
            {
                ...
                using (var transaction = con.Connection.BeginTransaction())
                {
                    var query = con.Select("exec SP_Make_ups @Name, @ID", new
                    {
                        Name = name,
                        ID = id
                    }, transaction);                    
                ...
                bitacora.Insert($"Ins/Upd", "SP_Make_ups", UserId);
